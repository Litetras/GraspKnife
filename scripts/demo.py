...
class Grasp:
    ...
    抓取结果结构化封装类
    属性说明：
    - pose: 4x4 齐次变换矩阵（抓取位姿，世界坐标系）
    - score: 抓取质量分数（0-1）
    - collision_free: 是否无碰撞（bool）
    - gripper_name: 夹爪名称
    - all_collision_free_grasps: 所有无碰撞抓取姿态列表（可选）
    ...
    def __init__(
        ...
        self.all_collision_free_grasps = all_collision_free_grasps  # 所有无碰撞抓取
...
def demo_variable(
        ...
        gripper_config: str = "/home/zyp/GraspGen/models/checkpoints/graspgen_robotiq_2f_140.yml",
    ...
    # 6. 物体点云下采样（解决CUDA OOM）
    if len(object_pc) > max_object_points:
        indices = np.random.choice(len(object_pc), max_object_points, replace=False)
        object_pc = object_pc[indices]
        if object_colors is not None:
            object_colors = object_colors[indices]
        print(f"Downsampled object PC: {len(object_pc) + len(indices)} → {max_object_points} points")
    else:
        print(f"Object PC points: {len(object_pc)} (no downsampling)")
    ...
    print(
        f"Inferred {len(grasps_inferred)} grasps (score range: {grasp_conf_inferred.min():.3f}~{grasp_conf_inferred.max():.3f})")
    ...
    if visualize and vis is not None:
        ...
        for i, (grasp, color) in enumerate(zip(collision_free_grasps, collision_free_colors)):
            visualize_grasp(vis, f"collision_free_grasps/{i:03d}", grasp, color, gripper_name, linewidth=0.8)
        # 可视化碰撞抓取（前20个）
        colliding_grasps = grasps_centered[~collision_free_mask]
        for i, grasp in enumerate(colliding_grasps[:20]):
            visualize_grasp(vis, f"colliding_grasps/{i:03d}", grasp, [255, 0, 0], gripper_name, linewidth=0.4)
