...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        #required=True,
        default='/home/zyp/GraspGen/scripts/data1/depth.png',
    ...
    parser.add_argument(
        ...
        type=str,
        #required=True,
        default='/home/zyp/GraspGen/scripts/data1/color_mask.png',
        help="Path to the instance segmentation mask (e.g., .npy or image file)",
    )
    parser.add_argument(
        "--rgb_image_path",
        type=str,
        default='/home/zyp/GraspGen/scripts/data1/color.png',  #default=None,
    ...
    parser.add_argument(
        ...
        default=[605.26, 605.34, 318.31, 234.76],#?????????????
    ...
    parser.add_argument(
        ...
        type=int,
        default=30000,
        help="Maximum number of object points to use for grasp inference (to avoid CUDA errors)",
    )
    parser.add_argument(
        "--visualize",
        action="store_true",
...
########################################################################################################################
    # 新增：对物体点云进行下采样，限制数量（解决CUDA错误的核心步骤）
    if len(object_pc) > args.max_object_points:
        # 随机下采样（保持均匀性）
        indices = np.random.choice(len(object_pc), args.max_object_points, replace=False)
        object_pc = object_pc[indices]
        # 同步下采样颜色信息（如果存在）
        if object_colors is not None:
            object_colors = object_colors[indices]
        print(f"Downsampled object point cloud from {len(object_pc) + len(indices)} to {args.max_object_points} points")
    else:
        print(f"Object point cloud has {len(object_pc)} points (no downsampling needed)")
########################################################################################################################
    ...
    print(
        f"Filtered object point cloud: {len(pc_filtered)} points (removed {len(pc_removed)} outliers)"
    )
