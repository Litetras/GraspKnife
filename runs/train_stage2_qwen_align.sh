...
# Stage II: align Qwen-LoRA natural language to the frozen CLIP strict prior.
...
export LOG_DIR="${LOG_DIR:-$RESULTS_DIR/logs/stage2_qwen_align}"
export STAGE1_CHECKPOINT="${STAGE1_CHECKPOINT:-$RESULTS_DIR/logs/stage1_clip_prior/last.pth}"
export CHECKPOINT="${CHECKPOINT:-$STAGE1_CHECKPOINT}"
export CACHE_DIR="${CACHE_DIR:-$RESULTS_DIR/cache}"

echo "Running Stage II Qwen anchor alignment for $GRIPPER_NAME"
echo "Initializing from: $CHECKPOINT"
...
python train_graspgen.py \
    ...
    +train.reset_epoch_on_load=True \
