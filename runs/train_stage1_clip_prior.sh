...
# Stage I: CLIP strict phrase prior.
# 默认训练 generator + discriminator；如需单独跑：
#   TARGET=gen bash runs/train_stage1_clip_prior.sh
#   TARGET=dis bash runs/train_stage1_clip_prior.sh
export LOD_LANGUAGE_MODE=clip_strict
export GRASPGEN_LANGUAGE_MODE="${GRASPGEN_LANGUAGE_MODE:-clip_strict}"
...
export GRASPGEN_DISABLE_POSE_METRICS="${GRASPGEN_DISABLE_POSE_METRICS:-1}"
...
export GEN_NEPOCH="${GEN_NEPOCH:-6000}"
export GEN_BATCH="${GEN_BATCH:-16}"
export GEN_PLOT_FREQ="${GEN_PLOT_FREQ:-1000}"
export GEN_SAVE_FREQ="${GEN_SAVE_FREQ:-1000}"
export GEN_EVAL_FREQ="${GEN_EVAL_FREQ:-50}"
...
export GEN_LOG_DIR="${GEN_LOG_DIR:-$RESULTS_DIR/logs/stage1_clip_prior_gen}"
export GEN_CHECKPOINT="${GEN_CHECKPOINT:-$GEN_LOG_DIR/last.pth}"

export DIS_NEPOCH="${DIS_NEPOCH:-8000}"
export DIS_BATCH="${DIS_BATCH:-16}"
export DIS_PLOT_FREQ="${DIS_PLOT_FREQ:-100}"
...
export DIS_LR="${DIS_LR:-0.00002}"
export DIS_RATIO="${DIS_RATIO:-[0.50,0.45,0.00,0.05,0.00,0.00,0.00]}"
export DIS_LOG_DIR="${DIS_LOG_DIR:-$RESULTS_DIR/logs/stage1_clip_prior_dis}"
...
run_generator() {
    echo "Running Stage I generator: CLIP strict phrase prior"
    mkdir -p "$GEN_LOG_DIR"
...
run_discriminator() {
    echo "Running Stage I discriminator: CLIP strict phrase prior"
    mkdir -p "$DIS_LOG_DIR"
    python train_graspgen.py \
