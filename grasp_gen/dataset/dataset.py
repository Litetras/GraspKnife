...
class PickDataset(Dataset):
    def __init__(
       ...
       # ================= 新增：读取 JSON 任务文本 =================
        ...
        if len(self.scenes) <= 10:  # If the dataset is very small, we are likely in overfitting/debugging mode. In that case, we can repeat the scenes multiple times to allow for more iterations per epoch, which can help with convergence and debugging.
