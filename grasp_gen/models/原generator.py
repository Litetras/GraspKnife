...
#     组合模式（compositional_schedular）：位置用 scaled_linear，旋转用 squaredcos_cap_v2，两个独立调度器 generator.py:160-185
    ...
    def forward_train(self, data):
        ...
        noise = torch.randn([batch_size, self.output_dim], device=device).float()#生成噪声

        timesteps = torch.randint(#采样一个随机时间步
        ...
        if self.compositional_schedular:
            ...
            noisy_grasps = self.noise_scheduler.add_noise(grasps_gt, noise, timesteps)#添加噪声

        samples = noisy_grasps if self.pose_repr == "mlp" else None
        noise_pred = self.diffusion_head(object_embedding, timesteps, samples)#预测噪声###################
...
#     L1 Rotation Loss：旋转 L1 损失 generator.py:376-397
        ...
        if self.loss_pointmatching:
            point_matching_loss = compute_grasp_loss(#计算点匹配损失，输入是实际噪声点云、预测噪声点云和控制点，控制点是根据夹爪模型预定义的一组点，用于评估抓取姿态的物理合理性
    ...
    def forward_inference(self, data, return_metrics=False):
        ...
        with torch.no_grad():
            ...
            for k in timesteps:
                ...
                noise_pred = self.diffusion_head(object_embedding, k, samples)#预测噪声
...
#还可选配注意力机制（attention参数），支持 self-attention 或 cross-attention Transformer Decoder，含 3 层、8 个头、512 维前馈网络。
        if self.attention.find("attn") > 0:
            ...
            #3层Self-Attention块：每层包含AttentionLayer + FFNLayer generator.py:749-760
            else:
