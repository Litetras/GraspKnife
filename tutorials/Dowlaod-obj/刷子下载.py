...
# 配置区域 (请根据需要修改路径)
# ==========================================
UID_LIST_FILE = "dust_bed_brush_uids.json"  # 刚才筛选好的刷子 UID 列表
...
print(f"已加载 {len(brush_uids)} 个手持除尘刷/床刷 UID。")

# 可选：如果你只想先测试下载前10个，取消下面这行的注释
# brush_uids = brush_uids[:10]
...
for uid, glb_path in objects.items():
    try:
        ...
        # 导出文件，改名为 dust_brush_xxx
        obj_filename = f"dust_brush_{uid[:8]}.obj"
